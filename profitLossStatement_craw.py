...
class Crawler():
  def __init__(self):
    ...
    self.url = "https://mops.twse.com.tw/mops/web/ajax_t164sb04"
  ...
  def __get_data2(self, stock_no, year, season):
    res2 = self.req.post(self.url, {"encodeURIComponent":1,
                                    "step":2,
                                    "firstin":1,
                                    "TYPEK":"sii",
                                    "co_id": stock_no,
...
def implement(stock_no, year, season, path, trash_path):
  try:
    ...
    print("{}/{}/{}_Error {}: {}".format(stock_no, year, season, datetime.now(), e), flush=True)
    time.sleep(60)
...
def stocks(stock_info):
  ...
  for Y in range(102, year_now+1):
    ...
    for S in range(1, ED_S+1):
      PATH = "../stock_data/profitLoss_statement/{2} {3}/{0}_{1}.json".format(Y+1911,S,stock_no,stock_name)
      trash_PATH = "../stock_data/profitLoss_statement/TRASH/{2} {3}/{0}_{1}.json".format(Y+1911,S,stock_no,stock_name)
      # 判斷路徑是否存在
...
with open("../stock_data/stock_list/OTC_stock_info_list.json","r",encoding="utf-8") as o:
...
day_now = today.day
season_now = {
  1:4, 2:4, 3:4, 4:1, 5:1, 6:1, 7:2, 8:2, 9:2, 10:3, 11:3, 12:3
}
