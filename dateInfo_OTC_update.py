...
with open("../stock_data/stock_list/OTC_stock_info_list.json","r",encoding="utf-8") as f:
...
for stock_info in stock_info_list["stock"]:
  ...
  start_PATH = "../stock_data/date_info/OTC_stocks/{2} {3}/{0}_{1:02}.json".format(start_year,start_month,stock_no,stock_name)
  ...
  PATH = "../stock_data/date_info/OTC_stocks/{2} {3}/{0}_{1:02}.json".format(year,month,stock_no,stock_name)
  ...
  judge_json = "{}_{:02}.json".format(year if month<12 else year+1,month+1 if month<12 else 1)

  if last_json != today_json:
    #先判斷是否有這個月的資料，沒有就更新到這個月為止
    while last_json != judge_json:
      last_path = "../stock_data/date_info/OTC_stocks/{} {}/{}".format(stock_no,stock_name,last_json)
