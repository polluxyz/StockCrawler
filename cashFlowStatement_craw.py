...
def stocks(stock_info):
  ...
  for Y in range(102, year_now+1):
    ...
    for S in range(1, ED_S+1):
      PATH = "../stock_data/cashFlow_statement/{2} {3}/{0}_{1}.json".format(Y+1911,S,stock_no,stock_name)
      trash_PATH = "../stock_data/cashFlow_statement/TRASH/{2} {3}/{0}_{1}.json".format(Y+1911,S,stock_no,stock_name)
      # 判斷路徑是否存在
